...
超过七年软件开发经验，熟悉C/C++语言，熟悉高性能网络并发，Poco库等；熟练掌握shell，熟悉Git开发管理及redis使用等；

了解grpc/json-rpc/rest等技术，了解微服务设计等；对数据可视化，跨平台应用开发，openstack云计算等新技术感兴趣
...
* Shell脚本开发，自动化和文本处理
* 跨平台GUI程序开发（electron&vue&grpc/json-rpc&c++）
* 数据可视化展示技术
...
* 项目简介：
  ...
  通过事件驱动和数据驱动的方式，实现了任务的生成和管理；服务器内部使用消息队列驱动消息循环，redis缓存数据等技术；与CMS之间使用json通信(类REST风格)；
...
* 项目简介：
  ...
  对图片进行去噪去背景去logo等处理，用轮廓识别主体，以提高SIFT算法图像匹配率；使用轮廓和中心范围阈值优化SIFT匹配等；
  
  批量处理转换成不同尺寸的图片，以适应各种不同平台的展示需求；代码跨平台编译等。基于opencv视觉库开发，可运行于多个平台(linux/cygwin/win)。
